<?php 
$id=$_POST['id_product'];
if(empty($_POST['name_pro']) || empty($_POST['product_size']) || empty($_POST['price']) || empty($_POST['description']) || empty($_POST['type_id'])) {
...
if(count($size)!=count($price)){
	$error="Kích thước và giá sản phẩm không hợp lệ";
	header("Location: ./update_product.php?id=$id&error=$error");
	exit();
}
//check pic
